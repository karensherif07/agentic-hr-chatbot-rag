...
def _load_pdf_entries_from_db(lang: str) -> list:
    try:
        with get_db() as db:
            rows = db.execute(sql_text("""
                SELECT file_path, doc_name FROM policy_documents
                WHERE lang = :lang AND is_active = TRUE
                ORDER BY doc_name
            """), {"lang": lang}).fetchall()
        ...
        print(f"[setup] Could not load policy_documents from DB ({e}); "
              f"falling back to hardcoded {lang} PDF list.")
    if lang == "arabic":
        return [
