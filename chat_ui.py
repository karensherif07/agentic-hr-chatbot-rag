...
def save_and_summarise(
    ...
    summary_llm,        # ← now receives the correct language LLM (not always en_llm)
    employee_id: int,
):
    """
    Append messages to history, summarise every 8 turns, persist to DB.
    summary_llm should be ar_llm for Arabic/Franco conversations so the
    summary stays in the same language as the conversation.
    """
    ...
    save_session(employee_id, st.session_state.chat_history,
                 st.session_state.conversation_summary)
