"""
personal_data.py
Fetches an employee's live data from PostgreSQL.
Returns plain dicts — no raw SQLAlchemy Row objects.
"""
...
def get_employee_profile(employee_id: int) -> dict:
    ...
    if not r:
        return {}
    d = _row(r)
    # Explicit computed field — LLM must use this, not infer from other fields
    d["probation_status"] = f"ACTIVE — ends {d['probation_end_date']}" if d.get("probation_end_date") else "NOT IN PROBATION"
    return d
    ...
    with get_db() as db:
        rows = db.execute(text("""
            SELECT lr.id, lr.leave_type, lr.start_date, lr.end_date,
                   ...
                   a.full_name AS approved_by_name, lr.rejection_reason
    ...
    with get_db() as db:
        rows = db.execute(text("""
            SELECT pr.review_period, pr.review_date, pr.rating,
                   pr.rating_label, pr.salary_increment_pct, pr.bonus_multiplier,
                   pr.strengths, pr.areas_for_growth, pr.overall_comments,
...
def get_performance_trend(employee_id: int) -> dict:
    ...
    ratings = [r["rating"] for r in reviews][::-1]
    avg = round(sum(ratings) / len(ratings), 1)
    direction = "stable"
    if len(ratings) >= 2:
        if ratings[-1] > ratings[0]:   direction = "improving"
        elif ratings[-1] < ratings[0]: direction = "declining"
    return {
    ...
    with get_db() as db:
        if period:
            rows = db.execute(text("""
                ...
                FROM okrs WHERE employee_id = :eid AND period = :period
            ...
            rows = db.execute(text("""
                ...
                WHERE employee_id = :eid
                  AND period = (SELECT period FROM okrs WHERE employee_id = :eid ORDER BY set_date DESC LIMIT 1)
                ORDER BY set_date DESC
...
def get_latest_salary(employee_id: int) -> dict | None:
    with get_db() as db:
        r = db.execute(text("""
            ...
            FROM payroll_records WHERE employee_id = :eid ORDER BY month DESC LIMIT 1
        """), {"eid": employee_id}).fetchone()
...
def get_payroll_history(employee_id: int, months: int = 6) -> list[dict]:
    with get_db() as db:
        rows = db.execute(text("""
            ...
            FROM payroll_records WHERE employee_id = :eid ORDER BY month DESC LIMIT :m
...
def get_training_record(employee_id: int, year: int = None) -> dict | None:
    ...
    with get_db() as db:
        r = db.execute(text("""
            SELECT year, budget_total_usd, budget_used_usd, training_days_total,
                   training_days_used, courses, scholarship_applied,
            ...
            FROM training_records WHERE employee_id = :eid AND year = :yr
...
def get_active_disciplinary(employee_id: int) -> list[dict]:
    with get_db() as db:
        rows = db.execute(text("""
            ...
            FROM disciplinary_records WHERE employee_id = :eid AND is_active = TRUE
...
def fetch_for_intent(employee_id: int, intent_topic: str) -> dict:
    """Fetches only the data relevant to the intent topic."""
    today = date.today()
    fetchers = {
        "leave":        lambda: {"leave_balances": get_leave_balance(employee_id, today.year),
                                  "pending_leaves": get_pending_leave(employee_id),
                                  "recent_requests": get_leave_requests(employee_id, limit=5)},
        "performance":  lambda: {"performance_history": get_performance_history(employee_id),
                                  "performance_trend":   get_performance_trend(employee_id),
                                  "latest_review":       get_latest_review(employee_id)},
        "salary":       lambda: {"latest_salary": get_latest_salary(employee_id),
    ...
    data = fn()
    data["profile"] = get_employee_profile(employee_id)   # always included
