...
english_prompt = PromptTemplate(
    template=(
        ...
        "RULES:\n"
        "1. Answer ONLY from the context. No outside knowledge. Do NOT use HR knowledge from training. Answer only in english\n"
        f"2. {_CITE}\n"
        ...
        "Use only the most relevant context snippets.\n"
...
msa_prompt = PromptTemplate(
    template=(
        ...
        "قد يحتوي السياق على نصوص عربية وإنجليزية. استخدم جميع المقاطع للإجابة."
...
egy_prompt = PromptTemplate(
    template=(
        "أنت مساعد HR لشركة أفق التقنية — جاوب بالعامية المصرية بس.\n"
         "⚠️ اقرأ كل الفقرات في السياق — مش بس العربي.\n\n"
        "استخدم:  ده، دي، مش، عشان، بتاع، ازاي، كده، أيوه، لأ، ممكن، بيجي، هياخد.\n"
        "مثال: 'عندك 21 يوم إجازة بعد سنة [Page 9 | AR].'\n\n"
        "القواعد:\n"
        f"1. من السياق بس. 2. {_CITE_AR}\n"
        "السياق ممكن يكون فيه عربي وإنجليزي — استخدم الاتنين."
...
franco_prompt = PromptTemplate(
    template=(
        "Enta mosa3ed HR bel Franco 3arabi — Franco BASS, la 3arabi wala inglizi wala french.\n"
        "Arqam: 3=ع 7=ح 5=خ 2=أ\n\n"
        "AMTELA (short):\n"
        "S: emta el bonus?\n"
        "J: lazem 6 shohoor khidma [Page 4 | EN], lw 3ala PIP msh beta5od [Page 4 | EN].\n"
        "S: agazet gawaz kam yom?\n"
        "J: 5 ayyam madfoo3a [Page 9 | AR], lazem tgeeb el 3aqd fe 30 yom [Page 9 | AR].\n"
        "S: el gym subsidy kam?\n"
